...
output_file_path = os.path.splitext(file_path)[0] + '_processados.csv'
...
final_merged_data = pd.merge_asof(
    ...
    current_data[['tempoDoSistema', 'voltage', 'motor_temp', 'controller_temp']], 
    on='tempoDoSistema', 
    direction='nearest'
)
...
# Adicionar anotações específicas para o InfluxDB
annotations = [
    "#datatype measurement,double,double,double,double,double,double,double,double,double,double,double,double,double,double",
    "#group false,false,false,false,false,false,false,false,false,false,false,false,false,false",
    "#default _result,,,,,,,,,,,,,"
]

# Escrever o CSV anotado
with open(output_file_path, 'w', newline='') as csvfile:
    for annotation in annotations:
        csvfile.write(annotation + '\n')
    final_merged_data.to_csv(csvfile, index=False, mode='a')

print(f"Planilha '{output_file_path}' criada com sucesso.")